...
const handleUpdate = async (update) => {
  ...
  try {
    ...
    try { await writeAudit({ commandId, operatorId, operatorRole: role, command: spec.command, status: 'failed' }); } catch {}
...
const run = async () => {
  ...
  while (true) {
    try {
      const updates = await telegramApi('getUpdates', { offset, timeout: POLL_TIMEOUT_SECONDS, allowed_updates: ['message'] });
