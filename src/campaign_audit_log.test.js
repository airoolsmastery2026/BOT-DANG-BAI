...
describe('campaign audit log', () => {
  ...
  test('does not duplicate the same audit event id', () => {
    const entry = { id: 'stable-event', campaignId: 'campaign-1', event: 'campaign.persisted' };
  ...
  test('creates workflow-scoped audit events with orchestrator run id', () => {
    appendWorkflowAuditEvent({ campaign: { id: 'campaign-2' }, orchestrator: { runId: 'run-2' } }, 'campaign.approved', {
      id: 'approval-1', status: 'approved', source: 'review',
    });

    expect(getCampaignAuditLog()[0]).toMatchObject({
      campaignId: 'campaign-2', runId: 'run-2', event: 'campaign.approved', status: 'approved', source: 'review',
    });
  });
