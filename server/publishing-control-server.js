...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/v1/publishing/health') {
    return json(res, 200, {
      schemaVersion: '1.0',
      requestId: String(req.headers['x-dhp-request-id'] || crypto.randomUUID()),
      data: { status: 'ok', scheduler: runtime.getState() },
    });
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/v1/publishing/scheduler/pause') {
    ...
    return json(res, 200, {
      schemaVersion: '1.0',
      requestId: String(req.headers['x-dhp-request-id'] || crypto.randomUUID()),
      data: runtime.pause(contextFrom(req)),
    });
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/v1/publishing/scheduler/resume') {
    ...
    return json(res, 200, {
      schemaVersion: '1.0',
      requestId: String(req.headers['x-dhp-request-id'] || crypto.randomUUID()),
      data: runtime.resume(contextFrom(req)),
    });
  }
